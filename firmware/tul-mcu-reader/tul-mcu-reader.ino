...
static void drawProSpeaker(int x, int y, bool left) {
    ...
    const uint16_t g = 0x5AEB;
    const uint16_t c = TFT_CYAN;
    if (left) {
        tft.fillRect(x, y + 15, 10, 30, g);
        tft.fillRect(x + 10, y + 5, 4, 50, w);
        tft.fillTriangle(x + 14, y + 5, x + 40, y - 5, x + 40, y + 65, w);
        tft.fillRoundRect(x + 40, y - 5, 8, 70, 4, w);
        tft.fillCircle(x + 40, y + 30, 8, c);
    } else {
        tft.fillRect(x + 50, y + 15, 10, 30, g);
        tft.fillRect(x + 46, y + 5, 4, 50, w);
        tft.fillTriangle(x + 46, y + 5, x + 20, y - 5, x + 20, y + 65, w);
        tft.fillRoundRect(x + 12, y - 5, 8, 70, 4, w);
        tft.fillCircle(x + 20, y + 30, 8, c);
    }
}
...
void setup() {
    ...
    Serial.printf("Touch controller: FT5x06 I2C=0x38 SDA=%d SCL=%d INT=%d\n", 6, 5, 7);
